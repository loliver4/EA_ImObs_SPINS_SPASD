...
#SBATCH --output=/projects/imoxonemre/SPIN_ASD/imobs_PALM_transdiag/imobs_transdiag_mentalizing_cov_age_sex/PALM_logs/PALM_transdiag_imobs_task_mentalizing_cov_age_sex%j.txt
#SBATCH --error=/projects/imoxonemre/SPIN_ASD/imobs_PALM_transdiag/imobs_transdiag_mentalizing_cov_age_sex/PALM_logs/PALM_transdiag_imobs_task_mentalizing_cov_age_sex%jerr.txt
#SBATCH --array=1
...
# assign the current directory variable and paths to the filielist and sublist for this 1 group analysis (ASD, SSD and TDC as one group)

DIR="/projects/imoxonemre/SPIN_ASD/imobs_PALM_transdiag/imobs_transdiag_mentalizing_cov_age_sex"

sublistids="${DIR}/subids/ASD_SSD_TDC_sublist_imobs.txt"
filename="$(find $DIR/filelists/ASD_SSD_TDC/filelist* -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"
truncname="$(echo $(basename `echo "$filename"`) | sed 's/filelist/ASD_SSD_TDC/;s/.txt//')"
outdir="${DIR}/results/$truncname"
# need a contrast and design matrix this time
desmat="$DIR/con_design/transdiag_imobs_task_design_fscore_mentalizing_cov_age_sex.csv"
conmat="$DIR/con_design/1_group_imobs_task_contrast_mentalizing_cov_age_sex.csv"
...
runpalm() {
    ...
    # C1 = positive slope; C2 = negative slope; C3 = positive mentalizing; c4 = negative mentalizing; C5 = positive age; C6 = negative age; C7 = positive sex; C8 = negative sex
