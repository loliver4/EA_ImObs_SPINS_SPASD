...
#SBATCH --output=/projects/imoxonemre/SPIN_ASD/imobs_PALM_1_group/pos_imobs_1_group_cov_age_sex/PALM_logs/PALM_TDC_pos_imobs_task_cov_age_sex%j.txt
#SBATCH --error=/projects/imoxonemre/SPIN_ASD/imobs_PALM_1_group/pos_imobs_1_group_cov_age_sex/PALM_logs/PALM_TDC_pos_imobs_task_cov_age_sex%jerr.txt
#SBATCH --array=1
...
# assign the current directory variable and paths to the filielist and sublist for this 1 group analysis (TDC)

DIR="/projects/imoxonemre/SPIN_ASD/imobs_PALM_1_group/pos_imobs_1_group_cov_age_sex"

sublistids="${DIR}/subids/TDC_sublist_imobs.txt"
filename="$(find $DIR/filelists/TDC/filelist* -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"
truncname="$(echo $(basename `echo "$filename"`) | sed 's/filelist/TDC/;s/.txt//')"
outdir="${DIR}/results/$truncname"
# need a contrast and design matrix this time
desmat="$DIR/con_design/TDC_imobs_task_design_cov_age_sex.csv"
conmat="$DIR/con_design/1_group_imobs_task_contrast_cov_age_sex.csv"
...
runpalm() {
    ...
    # C1 = positive slope; C2 = negative slope; C3 = positive age; c4 = negative age; C5 = positive sex; C6 = negative sex
