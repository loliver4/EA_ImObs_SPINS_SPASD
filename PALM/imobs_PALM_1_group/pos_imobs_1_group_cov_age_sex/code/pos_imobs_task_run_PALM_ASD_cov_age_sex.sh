...
#SBATCH --output=/projects/imoxonemre/SPIN_ASD/imobs_PALM_1_group/pos_imobs_1_group_cov_age_sex/PALM_logs/PALM_ASD_pos_imobs_task_cov_age_sex%j.txt
#SBATCH --error=/projects/imoxonemre/SPIN_ASD/imobs_PALM_1_group/pos_imobs_1_group_cov_age_sex/PALM_logs/PALM_ASD_pos_imobs_task_cov_age_sex%jerr.txt
#SBATCH --array=1
...
# assign the current directory variable and paths to the filielist and sublist for this 1 group analysis (ASD)

DIR="/projects/imoxonemre/SPIN_ASD/imobs_PALM_1_group/pos_imobs_1_group_cov_age_sex"

sublistids="${DIR}/subids/ASD_sublist_imobs.txt"
filename="$(find $DIR/filelists/ASD/filelist* -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"
truncname="$(echo $(basename `echo "$filename"`) | sed 's/filelist/ASD/;s/.txt//')"
outdir="${DIR}/results/$truncname"
# need a contrast and design matrix this time
desmat="$DIR/con_design/ASD_imobs_task_design_cov_age_sex.csv"
