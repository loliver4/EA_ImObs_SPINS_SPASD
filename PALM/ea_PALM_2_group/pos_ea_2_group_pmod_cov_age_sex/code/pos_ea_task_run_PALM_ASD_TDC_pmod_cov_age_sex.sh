...
#SBATCH --output=/projects/imoxonemre/SPIN_ASD/ea_PALM_2_group/pos_ea_2_group_pmod_cov_age_sex/PALM_logs/PALM_ASD_TDC_pos_ea_task_pmod_cov_age_sex%j.txt
#SBATCH --error=/projects/imoxonemre/SPIN_ASD/ea_PALM_2_group/pos_ea_2_group_pmod_cov_age_sex/PALM_logs/PALM_ASD_TDC_pos_ea_task_pmod_cov_age_sex%jerr.txt
#SBATCH --array=1
...
# assign the current directory variable and paths to the filielist and sublist for this 2 group analysis (ASD vs TDC)

DIR="/projects/imoxonemre/SPIN_ASD/ea_PALM_2_group/pos_ea_2_group_pmod_cov_age_sex"

sublistids="${DIR}/subids/ASD_TDC_sublist_ea.txt"
filename="$(find $DIR/filelists/ASD_TDC/filelist* -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"
truncname="$(echo $(basename `echo "$filename"`) | sed 's/filelist/ASD_TDC/;s/.txt//')"
outdir="${DIR}/results/$truncname"
# need a contrast and design matrix this time
desmat="$DIR/con_design/ASD_TDC_ea_task_design_cov_age_sex.csv"
...
runpalm() {
    ...
    # C1 = ASD > TDC; C2 = TDC > ASD; C3 = positive age; c4 = negative age; C5 = positive sex; C6 = negative sex
